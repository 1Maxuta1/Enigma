...
const FeatureCard: FC<FeatureCardProps> = ({ icon: Icon, title, content, index }) => (
  <div className={`rounded-lg hover:bg-purple-800 duration-300 transition flex p-6 ml-6 ${index !== features.length - 1 ? "mb-6" : "mb-0"}`} >
    <Icon size={100} className="bg-blue-500 rounded-full py-1 px-2 md:py-4 md:px-6 lg:py-6 lg:px-6 text-blue-200 max-w-[7rem] max-h-[7rem]" />
    <div className="pl-4">
      <div>{title}</div>
      <div>{content}</div>
    </div>
...
const Business: FC = () => {
  return (
    <section id="about" className={`text-white bg-gradient-to-br from-fuchsia-950 to-purple-950 px-16 flex w-full  md:flex-row flex-col ${styles.paddingY}`}>
